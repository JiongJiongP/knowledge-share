# Step 3: ContentEdit Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the ContentEdit.vue placeholder with a full edit page that reuses ContentCreate form structure, pre-filling existing content data fetched by route ID.

**Architecture:** Same form layout as ContentCreate. On mount, fetches content + tags by route `:id`. Pre-fills form fields. On submit, calls `updateContent` API. Supports publish from edit mode. Guards against editing non-owned content via API response.

**Tech Stack:** Vue 3 + Element Plus + Axios (no new deps)
...
| Action | File | Purpose |
|--------|------|---------|
| Replace | `frontend/src/views/ContentEdit.vue` | Full page implementation |

---

### Task 1: Implement ContentEdit page

**Files:**
- Replace: `frontend/src/views/ContentEdit.vue`

- [ ] **Step 1: Write ContentEdit.vue**
...
```bash
git add frontend/src/views/ContentEdit.vue
git commit -m "feat: implement ContentEdit page with pre-filled form and publish support"
```
