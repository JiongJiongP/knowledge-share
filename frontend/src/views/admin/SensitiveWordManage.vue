...
function categoryLabel(cat) {
  const map = { GENERAL: '通用', POLITICAL: '政治', ADVERTISING: '广告' }
  return map[cat] || cat
}

function categoryType(cat) {
  const map = { GENERAL: '', POLITICAL: 'danger', ADVERTISING: 'warning' }
  return map[cat] || 'info'
}
...
async function handleAdd() {
  ...
  saving.value = true
  try {
    await addSensitiveWord(addForm)
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm(`确定要删除"${row.word}"吗？`, '确认删除', { type: 'warning' })
...
async function handleBatchImport() {
  ...
  try {
    ...
    if (!list.length) throw new Error('无有效内容')
    await batchImportSensitiveWords({ words: list, category: batchCategory.value })
    ElMessage.success(`成功导入${list.length}个敏感词`)
